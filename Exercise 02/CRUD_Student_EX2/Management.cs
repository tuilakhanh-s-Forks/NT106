...
namespace CRUD_Student_EX2
{
	public partial class Management : Form
	{
		...
		private void LoadStudent(string query)
		{
			...
			DataBase.Command.CommandText = "SELECT ID, NAME, FORMAT(BIRTH,'dd/MM/yyyy') AS BIRTH, ADDRESS FROM STUDENT WHERE "
											+ "'" + query + "' = ''"
											+ " OR ID LIKE '%" + query + "%'"
											+ " OR NAME LIKE '%" + query + "%'"
											+ " OR CONVERT(VARCHAR, BIRTH, 103) LIKE '%" + query + "%'"
		...
		{
			LoadStudent(textBoxFindStudent.Text);
		}
